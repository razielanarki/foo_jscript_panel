...
function oItem(row_index, type, handle, track_index, group_index, track_index_in_group, heightInRow, groupRowDelta, obj, empty_row_index) {
	...
	this.drawRowContents = function (gr) {
		...
		var fader_txt = blendColors(txt_color, g_color_normal_bg, 0.35);

		if (is_item_selected) {
			gr.FillSolidRect(this.x + cover.w, this.y, this.w - cover.w, this.h, SetAlpha(g_color_selected_bg, 150));
			if (p.list.focusedTrackId == this.track_index) {
				gr.DrawRect(this.x + cover.w + 1, this.y, this.w - cover.w - 2, this.h, 1, SetAlpha(txt_color, 150));
			}
		}
		...
		for (var j = 0; j < p.headerBar.columns.length; j++) {
			...
			if (p.headerBar.columns[j].w > 0) {
				...
				case "State":
					...
					if (!is_item_selected)
						gr.FillSolidRect(cx, this.y + 2, cRow.playlist_h - 4, cRow.playlist_h - 4, txt_color & 0x08ffffff);

					gr.SetTextRenderingHint(4);
					if (fb.IsPlaying && plman.PlayingPlaylist == g_active_playlist && this.track_index == p.list.nowplaying.PlaylistItemIndex) {
						if (fb.IsPaused) {
							gr.DrawString(chars.pause, g_font_awesome_20, txt_color, cx, this.y + 2, cRow.playlist_h - 4, cRow.playlist_h - 4, cc_stringformat);
						} else {
							gr.DrawString(chars.play, g_font_awesome_20, g_seconds % 2 == 0 ? txt_color : SetAlpha(txt_color, 60), cx + 2, this.y + 2, cRow.playlist_h - 4, cRow.playlist_h - 4, cc_stringformat);
						}
					} else {
						var queue_index = get_tfo("[%queue_index%]").EvalActivePlaylistItem(this.track_index);
						if (queue_index.length) {
							gr.DrawRect(cx, this.y + 2, cRow.playlist_h - 4, cRow.playlist_h - 4, 1, txt_color);
							gr.GdiDrawText(queue_index, g_font_15_1, txt_color, cx + 1, this.y + 3, cRow.playlist_h - 4, cRow.playlist_h - 4, DT_CENTER | DT_VCENTER | DT_CALCRECT | DT_NOPREFIX | DT_SINGLELINE | DT_END_ELLIPSIS);
						}
					}
					break;
				case "Mood":
					...
					gr.SetTextRenderingHint(4);
					if (this.mood != 0) {
						gr.DrawString(chars.heart_on, g_font_awesome_20, this.mood_color, columns.mood_x, this.y, columns.mood_w, cRow.playlist_h, cc_stringformat);
					} else {
						gr.DrawString(chars.heart_off, g_font_awesome_20, txt_color & 0x20ffffff, columns.mood_x, this.y, columns.mood_w, cRow.playlist_h, cc_stringformat);
					}
					break;
				case "Rating":
					...
					gr.SetTextRenderingHint(4);
					gr.DrawString(chars.rating_off.repeat(5), g_font_awesome_20, txt_color & 0x20ffffff, columns.rating_x, this.y, columns.rating_w, cRow.playlist_h, lc_stringformat);
					gr.DrawString(chars.rating_on.repeat(this.rating), g_font_awesome_20, this.rating_color, columns.rating_x, this.y, columns.rating_w, cRow.playlist_h, lc_stringformat);
					break;
				default:
					if (p.headerBar.columns[j].tf != "null") {
						var line1 = get_tfo(p.headerBar.columns[j].tf).EvalActivePlaylistItem(this.track_index);
						DrawColoredText(gr, line1, g_font_12, txt_color, cx, tf1_y, cw, tf1_h, p.headerBar.columns[j].DT_align);
					}
					if (cList.enableExtraLine && p.headerBar.columns[j].tf2 != "null") {
						...
						DrawColoredText(gr, line2, g_font_12, fader_txt, cx, tf2_y, cw, tf2_h, p.headerBar.columns[j].DT_align);
					}
					break;
				}
			}
	...
	this.draw = function (gr, x, y, w, h) {
		...
		case 1:
			...
			// group header bg
			gr.FillSolidRect(this.x, (this.y - groupDelta) - 1 + this.h, this.w, 1, g_color_normal_txt & 0x25ffffff);
			gr.FillGradRect(this.x, (this.y - groupDelta) + 1, this.w, this.h - 2, 45, g_color_normal_txt & 0x15ffffff, 0, 1);
			...
			var fader_txt = blendColors(g_color_normal_txt, g_color_normal_bg, 0.35);
			...
			if (this.heightInRow == 1) {
				gr.GdiDrawText(this.l1 + " / " + this.l2, g_font_group1, g_color_normal_txt, this.x + cover.w + text_left_padding, (this.y - groupDelta) - 1, this.w - cover.w - text_left_padding * 4 - lg1_right_field_w - scrollbar_gap, this.h, DT_LEFT | DT_VCENTER | DT_CALCRECT | DT_NOPREFIX | DT_SINGLELINE | DT_END_ELLIPSIS);
				gr.GdiDrawText(this.r1, g_font_group1, g_color_normal_txt, this.x + cover.w + text_left_padding, (this.y - groupDelta) - 1, this.w - cover.w - text_left_padding * 5 + 2 - scrollbar_gap, this.h, DT_RIGHT | DT_VCENTER | DT_CALCRECT | DT_NOPREFIX | DT_SINGLELINE | DT_END_ELLIPSIS | DT_NOPREFIX);
				gr.FillSolidRect(this.x + cover.w + text_left_padding, Math.round(this.y + cRow.playlist_h * 1 - groupDelta - 5), this.w - cover.w - text_left_padding * 5 + 2 - scrollbar_gap, 1, g_line_colour);
				break;
			} else {
				gr.GdiDrawText(this.l1, g_font_group1, g_color_normal_txt, this.x + cover.w + text_left_padding, (this.y - groupDelta) + 3, this.w - cover.w - text_left_padding * 4 - lg1_right_field_w - scrollbar_gap, cRow.playlist_h, DT_LEFT | DT_VCENTER | DT_CALCRECT | DT_NOPREFIX | DT_SINGLELINE | DT_END_ELLIPSIS);
				gr.GdiDrawText(this.l2, g_font_group2, fader_txt, this.x + cover.w + text_left_padding, (this.y + cRow.playlist_h - groupDelta) - 4, this.w - cover.w - text_left_padding * 4 - lg2_right_field_w - scrollbar_gap, cRow.playlist_h, DT_LEFT | DT_VCENTER | DT_CALCRECT | DT_NOPREFIX | DT_SINGLELINE | DT_END_ELLIPSIS);
				gr.GdiDrawText(this.r1, g_font_group1, g_color_normal_txt, this.x + cover.w + text_left_padding, (this.y - groupDelta) + 3, this.w - cover.w - text_left_padding * 5 + 2 - scrollbar_gap, cRow.playlist_h, DT_RIGHT | DT_VCENTER | DT_CALCRECT | DT_NOPREFIX | DT_SINGLELINE | DT_END_ELLIPSIS);
				gr.GdiDrawText(this.r2, g_font_group2, fader_txt, this.x + cover.w + text_left_padding, (this.y + cRow.playlist_h - groupDelta) - 4, this.w - cover.w - text_left_padding * 5 + 1 - scrollbar_gap, cRow.playlist_h, DT_RIGHT | DT_VCENTER | DT_CALCRECT | DT_NOPREFIX | DT_SINGLELINE | DT_END_ELLIPSIS);
				gr.FillSolidRect(this.x + cover.w + text_left_padding, (this.y + cRow.playlist_h * 2 - groupDelta) - 8, this.w - cover.w - text_left_padding * 5 + 2 - scrollbar_gap, 1, g_line_colour);
				...
				if (this.heightInRow > 2 && this.obj) {
					...
					var lg3_right_field_w = gr.CalcTextWidth(lg3_right_field, g_font_12) + cList.borderWidth * 2;
					gr.GdiDrawText(lg3_left_field, g_font_12, fader_txt, this.x + cover.w + text_left_padding, (this.y + cRow.playlist_h * 2 - groupDelta) - 10, this.w - cover.w - text_left_padding * 4 - lg3_right_field_w - scrollbar_gap, cRow.playlist_h, DT_LEFT | DT_TOP | DT_VCENTER | DT_CALCRECT | DT_NOPREFIX | DT_SINGLELINE | DT_END_ELLIPSIS);
				}
			}
			...
			if (this.ishover && g_drag_drop_status && g_drag_drop_row_id > -1 && this.row_index == g_drag_drop_row_id) {
				gr.FillSolidRect(this.x, this.y - Math.floor(cList.borderWidth / 2), this.w, cList.borderWidth, g_color_selected_bg);
				gr.FillSolidRect(this.x, this.y - Math.floor(cList.borderWidth / 2) - 3 * cList.borderWidth, cList.borderWidth, 7 * cList.borderWidth, g_color_selected_bg);
				gr.FillSolidRect(this.x + this.w - cList.borderWidth, this.y - Math.floor(cList.borderWidth / 2) - 3 * cList.borderWidth, cList.borderWidth, 7 * cList.borderWidth, g_color_selected_bg);
			}
			break;
		}
	...
	this.check = function (event, x, y) {
		...
		case "up":
			...
			if (this.ishover) {
				if (rating_hover && this.metadb) {
					...
					if (foo_playcount) {
						if (new_rating != this.rating && new_rating > 0) {
							fb.RunContextCommandWithMetadb("Playback Statistics/Rating/" + new_rating, this.metadb);
						} else {
				...
				} else if (mood_hover && this.metadb) {
					var rp = this.metadb.RawPath;
					var can_tag = rp.indexOf("file://") == 0 || rp.indexOf("cdda://") == 0;
					...
					if (properties.use_foo_lastfm_playcount_sync) {
						...
						if (foo_lastfm) {
							fb.RunContextCommandWithMetadb("Last.fm/" + (loved == 1 ? "Unlove" : "Love"), this.metadb);
						} else if (foo_lastfm_playcount_sync) {
							fb.RunContextCommandWithMetadb("Last.fm Playcount Sync/" + (loved == 1 ? "Unlove" : "Love"), this.metadb);
						}
					} else if (can_tag) {
						var handles = fb.CreateHandleList(this.metadb);
		...
		case "move":
			...
			if (g_drag_drop_internal) {
				fb.DoDragDrop(plman.GetPlaylistSelectedItems(g_active_playlist), 1);
				g_drag_drop_internal = false;
			}
			break;
		}
...
function oList(object_name) {
	...
	this.init_groups = function (iscollapsed) {
		...
		for (var i = 0; i < this.count; i++) {
			var handle = this.handleList.Item(i);
			...
			if (this.totalRows > this.totalRowVisible) {
				...
				while (i < this.offset + total_rows_to_draw) {
					...
					if (this.s_groupheader_line_id >= 0) { // group header
						this.items.push(new oItem(row_index, 1, this.handleList.Item(this.s_track_id), this.s_track_id, this.s_group_id, 0, this.s_group_height, this.s_groupheader_line_id, this.groups[this.s_group_id], 0));
						...
						this.items.push(new oItem(row_index, 0, this.handleList.Item(this.s_track_id), this.s_track_id, this.s_group_id, track_index_in_group, 1, 0, null, this.s_delta));
						i++;
						row_index++;
					}
				}
	...
	this.draw = function (gr) {
		...
		if (g_drag_drop_status && g_drag_drop_bottom) {
			...
			gr.FillSolidRect(rx + cover.w, ry + item_height - Math.floor(cList.borderWidth / 2), rw - cover.w, cList.borderWidth, g_color_selected_bg);
			gr.FillSolidRect(rx + cover.w, ry + item_height - Math.floor(cList.borderWidth / 2) - 4 * cList.borderWidth, cList.borderWidth, 9 * cList.borderWidth, g_color_selected_bg);
		...
		if (this.drawRectSel) {
			...
			if (this.selX <= mouse_x) {
				if (this.selY - this.selDeltaRows * cRow.playlist_h <= mouse_y) {
					gr.FillSolidRect(this.selX, (this.selY - this.selDeltaRows * cRow.playlist_h), mouse_x - this.selX, mouse_y - (this.selY - this.selDeltaRows * cRow.playlist_h), g_color_selected_bg & 0x33ffffff);
					gr.DrawRect(this.selX, (this.selY - this.selDeltaRows * cRow.playlist_h), mouse_x - this.selX - 1, mouse_y - (this.selY - this.selDeltaRows * cRow.playlist_h) - 1, 1, g_color_selected_bg & 0x66ffffff);
				} else {
					gr.FillSolidRect(this.selX, mouse_y, mouse_x - this.selX, this.selY - mouse_y - this.selDeltaRows * cRow.playlist_h, g_color_selected_bg & 0x33ffffff);
					gr.DrawRect(this.selX, mouse_y, mouse_x - this.selX - 1, this.selY - mouse_y - this.selDeltaRows * cRow.playlist_h - 1, 1, g_color_selected_bg & 0x66ffffff);
				}
			} else {
				if (this.selY - this.selDeltaRows * cRow.playlist_h <= mouse_y) {
					gr.FillSolidRect(mouse_x, (this.selY - this.selDeltaRows * cRow.playlist_h), this.selX - mouse_x, mouse_y - (this.selY - this.selDeltaRows * cRow.playlist_h), g_color_selected_bg & 0x33ffffff);
					gr.DrawRect(mouse_x, (this.selY - this.selDeltaRows * cRow.playlist_h), this.selX - mouse_x - 1, mouse_y - (this.selY - this.selDeltaRows * cRow.playlist_h) - 1, 1, g_color_selected_bg & 0x66ffffff);
				} else {
					gr.FillSolidRect(mouse_x, mouse_y, this.selX - mouse_x, this.selY - mouse_y - this.selDeltaRows * cRow.playlist_h, g_color_selected_bg & 0x33ffffff);
					gr.DrawRect(mouse_x, mouse_y, this.selX - mouse_x - 1, this.selY - mouse_y - this.selDeltaRows * cRow.playlist_h - 1, 1, g_color_selected_bg & 0x66ffffff);
				}
			}
		}
	}
	...
	this.contextMenu = function (x, y, id, row_id) {
		...
		var colour_flag = properties.enableCustomColors ? MF_STRING : MF_GRAYED;
		_colours.AppendMenuItem(MF_STRING, 2, "Enable");
		_colours.CheckMenuItem(2, properties.enableCustomColors);
		...
		case 1:
			...
			break;
		case 2:
			properties.enableCustomColors = !properties.enableCustomColors;
			window.SetProperty("SYSTEM.Enable Custom Colors", properties.enableCustomColors);
			on_colours_changed();
			break;
		case 3:
			g_color_normal_txt = utils.ColourPicker(window.ID, g_color_normal_txt);
			window.SetProperty("SYSTEM.COLOUR TEXT NORMAL", g_color_normal_txt);
			on_colours_changed();
			break;
		case 4:
			g_color_normal_bg = utils.ColourPicker(window.ID, g_color_normal_bg);
			window.SetProperty("SYSTEM.COLOUR BACKGROUND NORMAL", g_color_normal_bg);
			on_colours_changed();
			break;
		case 5:
			g_color_selected_bg = utils.ColourPicker(window.ID, g_color_selected_bg);
			window.SetProperty("SYSTEM.COLOUR BACKGROUND SELECTED", g_color_selected_bg);
			on_colours_changed();
			break;
		case 10:
		...
		case 14:
			...
			break;
		case 15:
			var count = plman.PlaylistItemCount(g_active_playlist);
