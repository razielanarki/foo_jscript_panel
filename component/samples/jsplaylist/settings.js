...
function settings_radioboxes_action(id, status, parentId) {
	...
	case 3:
		if (id >= 1 && id <= 3) {
			...
			window.SetProperty("CUSTOM Wallpaper Type", properties.wallpapertype);
			images.wallpaper = get_wallpaper();
		}
		...
		if (id >= 5 && id <= 9) {
			...
			properties.wallpaperalpha = id * 25;
...
function settings_listboxes_action(pageId, id, selectedId) {
	...
	case 1:
		...
		case 0:
			...
			p.settings.pages[1].elements[6].status = tmp == 1; // Center align
			p.settings.pages[1].elements[7].status = tmp == 2; // Right align
...
function oCheckBox(id, x, y, label, linkedVariable, func, parentPageId) {
	...
	this.draw = function (gr) {
		...
		if (this.ly > cSettings.topBarHeight) {
			...
			gr.GdiDrawText(this.label, g_font_12_1, (this.status ? p.settings.color2 : p.settings.color1), label_x, this.ly, p.settings.w, p.settings.txtHeight + 10, g_LDT);
		}
	}
...
function oTextBox(id, x, y, w, h, label, value, func, parentPageId) {
	...
	this.draw = function (gr) {
		...
		if (this.ly + this.h > cSettings.topBarHeight) {
			gr.GdiDrawText(this.label, g_font_12_5, p.settings.color1, this.x, this.ly, p.list.w - p.settings.pages[this.parentPageId].scrollbarWidth - 10, this.h, g_LDT);
...
function oListBox(id, objectName, x, y, w, h, row_height, label, arr, selectedId, func, parentPageId) {
	...
	this.draw = function (gr) {
		...
		if (this.label.length > 0) {
			gr.GdiDrawText(this.label, g_font_12_5, p.settings.color1, this.x, this.ly - this.rowHeight - g_z2, this.w, this.rowHeight, g_LDT);
		}
		gr.FillSolidRect(this.x, this.ly, this.w, this.h + 1, RGB(240, 240, 240));
		gr.DrawRect(this.x - 1, this.ly - 1, this.w + 1, this.h + 2, 1, RGB(180, 180, 180));
		...
		for (var i = this.offset; i < this.max + this.offset; i++) {
			...
			gr.GdiDrawText((isCustom ? "[" : "") + this.arr[i] + (isCustom ? "]" : ""), i == this.selectedId ? g_font_12_1 : g_font_12, (i == this.selectedId ? RGB(0, 0, 0) : RGB(0, 0, 0)), this.x + g_z5, this.ly + row * this.rowHeight, this.w - this.scrollbarWidth - g_z10, this.rowHeight, g_LDT);
			row++;
		}
	}
...
function oPage(id, objectName, label, nbrows) {
	...
	this.init = function () {
		...
		case 3: // Appearance
			...
			if ([125, 150, 175, 200, 225].indexOf(properties.wallpaperalpha) == -1) {
	...
	this.draw = function (gr) {
		...
		switch (this.id) {
		case 0:
			gr.GdiDrawText("Layout", g_font_12_5, p.settings.color1, txtbox_x, cSettings.topBarHeight + rh * 1.5 - (this.offset * cSettings.rowHeight), p.settings.w - 10, p.settings.txtHeight + 10, g_LDT);
			break;
		case 1:
			var listBoxWidth = scale(100);
			var listBox_button_x = listBoxWidth + scale(50);
			gr.GdiDrawText("Status", g_font_12_5, p.settings.color1, txtbox_x, cSettings.topBarHeight + rh * 4.5 - (this.offset * cSettings.rowHeight), p.settings.w - 10, p.settings.txtHeight + 10, g_LDT);
			gr.GdiDrawText("Text Alignment", g_font_12_5, p.settings.color1, txtbox_x, cSettings.topBarHeight + rh * 14.5 - (this.offset * cSettings.rowHeight), p.settings.w - 10, p.settings.txtHeight + 10, g_LDT);
		...
		case 2:
			...
			gr.FillSolidRect(txtbox_x, cSettings.topBarHeight + rh * 8.5 - (this.offset * cSettings.rowHeight), ww - txtbox_x - 20 - this.scrollbarWidth, cHeaderBar.borderWidth, p.settings.color1);

			gr.GdiDrawText("Collapsed Row Height", g_font_12_5, p.settings.color1, txtbox_x, cSettings.topBarHeight + rh * 10.75 - (this.offset * cSettings.rowHeight), p.settings.w - 10, p.settings.txtHeight + 10, g_LDT);
			gr.GdiDrawText("Expanded Row Height", g_font_12_5, p.settings.color1, txtbox_x, cSettings.topBarHeight + rh * 12.75 - (this.offset * cSettings.rowHeight), p.settings.w - 10, p.settings.txtHeight + 10, g_LDT);
			gr.GdiDrawText("Cover Art Status", g_font_12_5, p.settings.color1, txtbox_x, cSettings.topBarHeight + rh * 14.75 - (this.offset * cSettings.rowHeight), p.settings.w - 10, p.settings.txtHeight + 10, g_LDT);
			gr.GdiDrawText("Auto-Collapse Status", g_font_12_5, p.settings.color1, txtbox_x, cSettings.topBarHeight + rh * 16.75 - (this.offset * cSettings.rowHeight), p.settings.w - 10, p.settings.txtHeight + 10, g_LDT);
			gr.GdiDrawText("Default Group Status", g_font_12_5, p.settings.color1, txtbox_x, cSettings.topBarHeight + rh * 18.75 - (this.offset * cSettings.rowHeight), p.settings.w - 10, p.settings.txtHeight + 10, g_LDT);
			break;
		case 3:
			gr.GdiDrawText("Wallpaper Status", g_font_12_5, p.settings.color1, txtbox_x, cSettings.topBarHeight + rh * 1.5 - (this.offset * cSettings.rowHeight), p.settings.w - 10, p.settings.txtHeight + 10, g_LDT);
			gr.GdiDrawText("Wallpaper Image", g_font_12_5, p.settings.color1, txtbox_x, cSettings.topBarHeight + rh * 3.25 - (this.offset * cSettings.rowHeight), p.settings.w - 10, p.settings.txtHeight + 10, g_LDT);
			gr.GdiDrawText("Wallpaper Effects", g_font_12_5, p.settings.color1, txtbox_x, cSettings.topBarHeight + rh * 5 - (this.offset * cSettings.rowHeight), p.settings.w - 10, p.settings.txtHeight + 10, g_LDT);
			gr.GdiDrawText("Wallpaper Alpha Shading", g_font_12_5, p.settings.color1, txtbox_x, cSettings.topBarHeight + rh * 6.75 - (this.offset * cSettings.rowHeight), p.settings.w - 10, p.settings.txtHeight + 10, g_LDT);
			break;
		}
...
function oSettings() {
	...
	this.setButtons = function () {
		...
		// Add a Custom Column
		var newColumn_off = gdi.CreateImage(rect_w, rect_h);
		gb = newColumn_off.GetGraphics();
		gb.DrawRect(0, 0, rect_w - 1, rect_h - 1, 1, this.color1);
		gb.SetTextRenderingHint(3);
		gb.DrawString("New Column", g_font_12_1, this.color2, 0, 0, rect_w, rect_h, cc_stringformat);
		newColumn_off.ReleaseGraphics(gb);

		var newColumn_ov = gdi.CreateImage(rect_w, rect_h);
		gb = newColumn_ov.GetGraphics();
		gb.DrawRect(1, 1, rect_w - 2, rect_h - 2, 2, this.color1);
		gb.SetTextRenderingHint(3);
		gb.DrawString("New Column", g_font_12_1, this.color2, 0, 0, rect_w, rect_h, cc_stringformat);
		newColumn_ov.ReleaseGraphics(gb);

		this.newbutton = new button(newColumn_off, newColumn_ov, newColumn_ov);

		// Delete a Custom Column
		var delColumn_off = gdi.CreateImage(rect_w, rect_h);
		gb = delColumn_off.GetGraphics();
		gb.DrawRect(0, 0, rect_w - 1, rect_h - 1, 1, this.color1);
		gb.SetTextRenderingHint(3);
		gb.DrawString("Delete Column", g_font_12_1, this.color2, 0, 0, rect_w, rect_h, cc_stringformat);
		delColumn_off.ReleaseGraphics(gb);

		var delColumn_ov = gdi.CreateImage(rect_w, rect_h);
		gb = delColumn_ov.GetGraphics();
		gb.DrawRect(1, 1, rect_w - 2, rect_h - 2, 2, this.color1);
		gb.SetTextRenderingHint(3);
		gb.DrawString("Delete Column", g_font_12_1, this.color2, 0, 0, rect_w, rect_h, cc_stringformat);
		delColumn_ov.ReleaseGraphics(gb);

		this.delbutton = new button(delColumn_off, delColumn_ov, delColumn_ov);

		// Add a Custom "Group By" Pattern
		var newPattern_off = gdi.CreateImage(rect_w, rect_h);
		gb = newPattern_off.GetGraphics();
		gb.DrawRect(0, 0, rect_w - 1, rect_h - 1, 1, this.color1);
		gb.SetTextRenderingHint(3);
		gb.DrawString("New Pattern", g_font_12_1, this.color2, 0, 0, rect_w, rect_h, cc_stringformat);
		newPattern_off.ReleaseGraphics(gb);

		var newPattern_ov = gdi.CreateImage(rect_w, rect_h);
		gb = newPattern_ov.GetGraphics();
		gb.DrawRect(1, 1, rect_w - 2, rect_h - 2, 2, this.color1);
		gb.SetTextRenderingHint(3);
		gb.DrawString("New Pattern", g_font_12_1, this.color2, 0, 0, rect_w, rect_h, cc_stringformat);
		newPattern_ov.ReleaseGraphics(gb);

		this.newbuttonPattern = new button(newPattern_off, newPattern_ov, newPattern_ov);

		// Delete a Custom "Group By" Pattern
		var delPattern_off = gdi.CreateImage(rect_w, rect_h);
		gb = delPattern_off.GetGraphics();
		gb.DrawRect(0, 0, rect_w - 1, rect_h - 1, 1, this.color1);
		gb.SetTextRenderingHint(3);
		gb.DrawString("Delete Pattern", g_font_12_1, this.color2, 0, 0, rect_w, rect_h, cc_stringformat);
		delPattern_off.ReleaseGraphics(gb);

		var delPattern_ov = gdi.CreateImage(rect_w, rect_h);
		gb = delPattern_ov.GetGraphics();
		gb.DrawRect(1, 1, rect_w - 2, rect_h - 2, 2, this.color1);
		gb.SetTextRenderingHint(3);
		gb.DrawString("Delete Pattern", g_font_12_1, this.color2, 0, 0, rect_w, rect_h, cc_stringformat);
		delPattern_ov.ReleaseGraphics(gb);

		this.delbuttonPattern = new button(delPattern_off, delPattern_ov, delPattern_ov);

		// Close Settings Button (BACK)
		var close_off = gdi.CreateImage(75, 75);
		gb = close_off.GetGraphics();
		gb.SetSmoothingMode(2);
		gb.DrawEllipse(3, 3, 66, 66, 6, this.color2);
		gb.DrawEllipse(0, 0, 72, 72, 1, RGBA(0, 0, 0, 100));
		gb.DrawEllipse(6, 6, 60, 60, 1, RGBA(0, 0, 0, 50));
		gb.FillSolidRect(18, 36, 39, 6, this.color2);
		...
		var close_ov = gdi.CreateImage(75, 75);
		gb = close_ov.GetGraphics();
		gb.SetSmoothingMode(2);
		gb.DrawEllipse(3, 3, 66, 66, 6, this.color1);
		gb.DrawEllipse(0, 0, 72, 72, 1, RGBA(0, 0, 0, 50));
		gb.DrawEllipse(6, 6, 60, 60, 3, RGBA(0, 0, 0, 25));
		gb.FillSolidRect(18, 36, 39, 6, this.color1);
		...
		close_ov.ReleaseGraphics(gb);

		button_zoomSize = scale(25);
		this.closebutton = new button(close_off.Resize(button_zoomSize, button_zoomSize, 7), close_ov.Resize(button_zoomSize, button_zoomSize, 7), close_ov.Resize(button_zoomSize, button_zoomSize, 7));
		...
		this.tab_width = "Appearance".calc_width(g_font_12_3) + cSettings.tabPaddingWidth;
		this.tab_height = this.txtHeight + scale(10) + cHeaderBar.borderWidth;
		this.tab_img = gdi.CreateImage(this.tab_width, this.tab_height);
		this.tabButtons = [];
	}
	...
	this.draw = function (gr) {
		...
		gr.FillSolidRect(this.x, this.y + (ty + this.tab_height), this.w - cScrollBar.width, 4, g_color_normal_bg);
		gr.FillGradRect(this.x, this.y + (ty + this.tab_height) + 4, this.w - cScrollBar.width, g_z10, 87, g_color_normal_bg, 0, 1);

		// draw top background
		gr.FillSolidRect(this.x, this.y, this.w, (ty + this.tab_height), blendColors(g_color_normal_bg, g_color_normal_txt, 0.05));
		gr.FillGradRect(this.x, this.y + (ty + this.tab_height) - 4, this.w, 4, 87, 0, RGBA(0, 0, 0, 20), 1);

		// draw close button
		this.closebutton.draw(gr, this.x + 13, this.y + 10);
		// draw Panel Title
		gr.GdiDrawText("Panel Settings", g_font_21_3, this.color2, this.x + this.closebutton.w + 20, this.y + 10, this.w - 50, g_font_21_3.Height, DT_LEFT | DT_VCENTER | DT_CALCRECT | DT_NOPREFIX | DT_SINGLELINE | DT_END_ELLIPSIS);

		// draw tabs
		gr.DrawLine(0, ty + this.tab_height, ww, ty + this.tab_height, 1, this.color1);
		for (var i = 0; i < this.tabButtons.length; i++) {
			var x = tx + (i * this.tab_width);
			if (i == this.currentPageId) {
				gr.DrawRect(x, ty, this.tab_width, this.tab_height, 1, this.color1);
				gr.FillSolidRect(x + 1, ty + 1, this.tab_width - 2, this.tab_height + 2, g_color_normal_bg);
				gr.GdiDrawText(this.pages[i].label, g_font_12_3, this.color2, x, ty, this.tab_width, this.tab_height, DT_CENTER | DT_VCENTER | DT_CALCRECT | DT_NOPREFIX | DT_SINGLELINE | DT_END_ELLIPSIS);
			} else {
				gr.GdiDrawText(this.pages[i].label, g_font_12_3, this.color1, x, ty, this.tab_width, this.tab_height, DT_CENTER | DT_VCENTER | DT_CALCRECT | DT_NOPREFIX | DT_SINGLELINE | DT_END_ELLIPSIS);
			}
			this.tabButtons[i].draw(gr, x, ty);
		}
	}
