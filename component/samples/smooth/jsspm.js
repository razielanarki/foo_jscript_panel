...
function oBrowser() {
	...
	this.init_groups = function () {
		...
		for (var i = 0; i < total; i++) {
			...
			if (g_filter_text.length == 0 || name.toLowerCase().indexOf(g_filter_text.toLowerCase()) > - 1) {
	...
	this.draw = function (gr) {
		...
		if (this.rows.length > 0) {
			...
			for (var i = g_start_; i <= g_end_; i++) {
				...
				gr.DrawString(plman.IsPlaylistLocked(this.rows[i].idx) ? chars.lock : chars.list, g_font_awesome, normal_text, ax, ay, ah, ah, cc_stringformat);

				if (this.inputboxID == i) {
					this.inputbox.draw(gr, ah, ay + 5);
				} else {
					gr.GdiDrawText(this.rows[i].name, g_font, normal_text, ah, ay, aw - (ah * 3), ah, DT_LEFT | DT_VCENTER | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
					gr.GdiDrawText(plman.PlaylistItemCount(this.rows[i].idx), g_font, fader_txt, ah, ay, aw - ah - 5, ah, DT_RIGHT | DT_VCENTER | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
				}
			}
		}
		...
		if (ppt.showHeaderBar) {
			...
			gr.GdiDrawText(boxText, g_font_box, blendColors(g_color_normal_txt, g_color_normal_bg, 0.3), 0, 0, ww - 5, ppt.headerBarHeight - 1, DT_RIGHT | DT_VCENTER | DT_CALCRECT | DT_NOPREFIX | DT_END_ELLIPSIS);
		}
	}
...
function on_mouse_move(x, y) {
	...
	// inputBox
	if (!cPlaylistManager.drag_moved && ppt.showHeaderBar) {
...
function g_sendResponse() {
	...
	brw.populate();
	if (brw.selectedRow < 0 || brw.selectedRow > brw.rows.length - 1)
		brw.selectedRow = 0;
}
	...
	if (y < brw.y) {
		...
		if (g_drag_drop_target_id > -1) {
			if (playlist_can_add_items(g_drag_drop_target_id)) {
				...
				action.Base = plman.PlaylistItemCount(g_drag_drop_target_id);
