ppt.tf_artist = fb.TitleFormat("[%artist%]");
ppt.tf_title = fb.TitleFormat("[%title%]");
ppt.tf_track = fb.TitleFormat("[%artist%] ^^ [%title%] ^^ [%tracknumber%] ^^ [%length%] ^^ $if2(%rating%,0)");
ppt.tf_time_remaining = fb.TitleFormat("[-%playback_time_remaining%]");
ppt.tf_groupkey = fb.TitleFormat(window.GetProperty("SMOOTH.GROUPKEY", "$if2(%album artist%,$if(%length%,%directory%,Stream)) ^^ $if2(%album%,$if(%length%,'('Singles')',%title%)) ^^ [%date%] ^^ [%genre%] ^^ [%discnumber%]"));
...
function oBrowser() {
	...
	this.init_groups = function () {
		...
		var arr = ppt.tf_groupkey.EvalWithMetadbs(this.list).toArray();
		var previous = "";
		var g = 0, t = 0;
	...
	this.get_track_tags = function (index) {
		var track_arr = ppt.tf_track.EvalWithMetadb(this.rows[index].metadb).split(" ^^ ");
		var tags = {
			artist : track_arr[0],
			title : track_arr[1],
	...
	this.draw = function (gr) {
		if (this.rows.length > 0) {
			...
			for (var i = g_start_; i <= g_end_; i++) {
				...
				case ppt.groupHeaderRowsNumber: // last group header row
					...
					gr.FillSolidRect(ax, ay, aw, ah * ppt.groupHeaderRowsNumber, g_color_normal_txt & 0x08ffffff);
					gr.DrawRect(ax, ay, aw, ah * ppt.groupHeaderRowsNumber, 1, g_color_normal_txt & 0x25ffffff);
					...
					if (ppt.groupHeaderRowsNumber == 1) {
						gr.GdiDrawText(this.groups[id].album_artist + " - " + this.groups[id].album, g_font_group1, fader_txt, ax + image_height + 5, ay, text_width - this.groups[id].date.calc_width(g_font_group1), ah, DT_LEFT | DT_VCENTER | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
						gr.GdiDrawText(this.groups[id].date, g_font_group1, g_color_normal_txt, 0, ay, aw - 5, ah, DT_RIGHT | DT_VCENTER | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
					} else {
						gr.GdiDrawText(this.groups[id].album_artist, g_font_group1, fader_txt, ax + image_height + 5, ay, text_width - this.groups[id].date.calc_width(g_font_group1), ah, DT_LEFT | DT_VCENTER | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
						gr.GdiDrawText(this.groups[id].album, g_font_group2, fader_txt, ax + image_height + 5, ay + g_font_group1.Height + 4, text_width - this.groups[id].genre.calc_width(g_font_group2), ah, DT_LEFT | DT_VCENTER | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);

						gr.GdiDrawText(this.groups[id].date, g_font_group1, g_color_normal_txt, 0, ay, aw - 5, ah, DT_RIGHT | DT_VCENTER | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
						gr.GdiDrawText(this.groups[id].genre, g_font_group2, g_color_normal_txt, 0, ay + g_font_group1.Height + 4, aw - 5, ah, DT_RIGHT | DT_VCENTER | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
					}
					break;
				case 0: // track row
					...
					if (ppt.showRating) {
						...
						gr.DrawString(chars.rating_off.repeat(5), g_font_awesome, SetAlpha(normal_text, 128), this.rating_x, ay, rw, ah, lc_stringformat);
						gr.DrawString(chars.rating_on.repeat(tags.rating), g_font_awesome, normal_text, this.rating_x, ay, rw, ah, lc_stringformat);
					}
					...
					if (isplaying) {
						...
						if (fb.IsPaused) {
							gr.DrawString(chars.pause, g_font_awesome, normal_text, ax, ay, ah, ah, cc_stringformat);
						} else {
							gr.DrawString(chars.play, g_font_awesome, g_seconds % 2 == 0 ? normal_text : SetAlpha(normal_text, 128), ax, ay, ah, ah, cc_stringformat);
						}
					}

					if (ppt.doubleRowText) {
						if (isplaying) {
							gr.GdiDrawText(g_time_remaining, g_font, normal_text, ax, ay, aw - 5, ah / 2, DT_RIGHT | DT_VCENTER | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
						} else {
							gr.GdiDrawText(tags.tracknumber, g_font, fader_txt, ax, ay, ah, ah / 2, DT_CENTER | DT_VCENTER | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
							gr.GdiDrawText(tags.length, g_font, normal_text, ax, ay, aw - 5, ah / 2, DT_RIGHT | DT_VCENTER | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
						}

						gr.GdiDrawText(title, g_font, normal_text, ax + ah, ay, aw - ah - rw - g_time_width - 10, ah / 2, DT_LEFT | DT_VCENTER | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
						gr.GdiDrawText(artist, g_font, fader_txt, ax + ah, ay + (ah / 2) - 2, aw - ah - rw - g_time_width - 10, ah / 2, DT_LEFT | DT_VCENTER | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
					} else {
						if (isplaying) {
							gr.GdiDrawText(g_time_remaining, g_font, normal_text, ax, ay, aw - 5, ah, DT_RIGHT | DT_VCENTER | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
						} else {
							gr.GdiDrawText(tags.tracknumber, g_font, fader_txt, ax, ay, ah, ah, DT_CENTER | DT_VCENTER | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
							gr.GdiDrawText(tags.length, g_font, normal_text, ax, ay, aw - 5, ah, DT_RIGHT | DT_VCENTER | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
						}

						gr.GdiDrawText(ppt.showArtistAlways ? artist + " - " + title : title, g_font, normal_text, ax + ah, ay, aw - ah - rw - g_time_width - 10, ah, DT_LEFT | DT_VCENTER | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
					}
					break;
				}
			}
		...
		if (ppt.showHeaderBar) {
			...
			gr.GdiDrawText(this.playlist_info, g_font_box, blendColors(g_color_normal_txt, g_color_normal_bg, 0.4), 0, 0, ww - 5, ppt.headerBarHeight - 1, DT_RIGHT | DT_VCENTER | DT_CALCRECT | DT_NOPREFIX | DT_END_ELLIPSIS);
		}
	}
			...
			if (rating_hover && this.rows[this.activeRow].metadb) {
				...
				if (foo_playcount) {
					if (new_rating != this.rows[this.activeRow].tags.rating && new_rating > 0) {
						fb.RunContextCommandWithMetadb("Playback Statistics/Rating/" + new_rating, this.rows[this.activeRow].metadb);
					} else {
	...
	this.context_menu = function (x, y, id, row_id) {
		...
		case idx == 6:
			var base = getFocusId();
			if (base == -1) {
				base = plman.PlaylistItemCount(g_active_playlist);
			} else {
				base++;
			}
			plman.UndoBackup(g_active_playlist);
		...
		case idx < 1000:
			var pl = idx - 100;
			plman.UndoBackup(pl);
			plman.InsertPlaylistItems(pl, plman.PlaylistItemCount(pl), items);
			break;
		case idx >= 1000:
	...
	this.settings_context_menu = function (x, y) {
		...
		_menu4.AppendMenuItem(ppt.doubleRowText ? MF_GRAYED : MF_STRING, 21, "Artist");
		...
		case 8:
			...
			break;
		case 9:
			var tmp = utils.InputBox(window.ID, "Enter the full path to an image.", window.Name, ppt.wallpaperpath);
...
function on_paint(gr) {
	...
	if (ppt.wallpapermode && g_wallpaperImg) {
		drawImage(gr, g_wallpaperImg, brw.x, brw.y, brw.w, brw.h, true, null, ppt.wallpaperalpha);
	}
	brw.draw(gr);
}
	...
	if (mask == KMask.none) {
		...
		if (vkey == 86) { // CTRL+V
			if (playlist_can_add_items(g_active_playlist) && fb.CheckClipboardContents()) {
				...
				plman.UndoBackup(g_active_playlist);
				plman.InsertPlaylistItems(g_active_playlist, plman.PlaylistItemCount(g_active_playlist), items, false);
				items.Dispose();
			}
		}
		if (vkey == 84) { // CTRL+T
...
function on_playback_dynamic_info_track(type) {
	if (type == 0) {
		g_radio_title = ppt.tf_title.Eval();
		g_radio_artist = ppt.tf_artist.Eval();
...
var g_radio_title = ppt.tf_title.Eval();
var g_radio_artist = ppt.tf_artist.Eval();
...
var g_filterbox = null; // recently removed but var must exist
