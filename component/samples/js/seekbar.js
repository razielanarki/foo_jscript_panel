function _seekbar(x, y, w, h, spectrogram_mode) {
	...
	this.paint = function (gr) {
		...
		if (this.spectrogram_working) {
			_drawImage(gr, this.hourglass_image, this.x, this.y, this.w, this.h, image.centre);
		} else if (this.spectrogram_image) {
			_drawImage(gr, this.spectrogram_image, this.x, this.y, this.w, this.h, image.stretch);
		}
		if (fb.IsPlaying && fb.PlaybackLength > 0) {
			gr.FillSolidRect(this.x + this.pos() - 2, this.y, 2, this.h, this.properties.marker_colour.value);
		}
	}
	...
	this.playback_new_track = function () {
		...
		if (metadb) {
			...
			case metadb.SubSong > 0:
				...
				break;
			case this.properties.library_only.enabled && !fb.IsMetadbInMediaLibrary(metadb):
	...
	this.rbtn_up = function (x, y) {
		...
		panel.s10.AppendMenuItem(MF_GRAYED, 1020, "In use: " + utils.FormatFileSize(fso.GetFolder(spectrogram_cache).Size));
	...
	this.rbtn_up_done = function (idx) {
		switch (idx) {
		case 1000:
			var tmp = utils.InputBox(window.ID, "All FFmpeg showspectrumpic options should work here.", window.Name, this.properties.params.value);
			...
			break;
		case 1001:
			var tmp = utils.ColourPicker(window.ID, this.properties.marker_colour.value);
			if (tmp != this.properties.marker_colour.value) {
				this.properties.marker_colour.value = tmp;
				_dispose(this.hourglass_image);
				this.hourglass_image = _chrToImg(chars.hourglass, this.properties.marker_colour.value);
				window.Repaint();
			}
			break;
		case 1002:
		...
		case 1012:
			...
			files.forEach(function (item) {
				if (idx == 1010 || (idx == 1011 && _fileExpired(item, ONE_DAY)) || (idx == 1012 && _fileExpired(item, ONE_DAY * 7))) _deleteFile(item);
			});
			break;
		}
	}
	...
	this.get_image = function (metadb) {
		...
		return gdi.Image(this.spectrogram_filename)
	}
	...
	if (this.spectrogram_mode) {
		...
		this.properties = {
			...
			marker_colour : new _p("2K3.SPECTROGRAM.MARKER.COLOUR", _RGB(240, 240, 240)),
