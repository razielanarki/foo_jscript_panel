		...
		case this.mode == 'musicbrainz' && this.properties.mode.value == 0: // releases
			...
			for (var i = 0; i < Math.min(this.items, this.rows); i++) {
				if (this.data[i + this.offset].url == 'SECTION_HEADER') {
					gr.GdiDrawText(this.data[i + this.offset].name, panel.fonts.title, panel.colours.highlight, this.x, this.y + _scale(12) + (i * panel.row_height), this.text_width, panel.row_height, LEFT);
				} else {
		...
		case this.mode == 'properties_other_info':
			...
			for (var i = 0; i < Math.min(this.items, this.rows); i++) {
				if (this.data[i + this.offset].value == 'SECTION_HEADER') {
					gr.GdiDrawText(this.data[i + this.offset].name, panel.fonts.title, panel.colours.highlight, this.x, this.y + _scale(12) + (i * panel.row_height), this.text_x - 10, panel.row_height, LEFT);
				} else {
					gr.GdiDrawText(this.data[i + this.offset].name, panel.fonts.normal, panel.colours.text, this.x, this.y + _scale(12) + (i * panel.row_height), this.text_x - 10, panel.row_height, LEFT);
					gr.GdiDrawText(this.data[i + this.offset].value, panel.fonts.normal, panel.colours.highlight, this.x + this.text_x, this.y + _scale(12) + (i * panel.row_height), this.text_width, panel.row_height, LEFT);
				}
			}
			break;
	...
	this.update = function () {
			...
			if (utils.IsFile(this.filename)) {
				this.data = _(_jsonParseFile(this.filename))
					.forEach(function (item) {
						item.width = item.name.calc_width(panel.fonts.normal);
					})
					.value();
			}
			break;
		...
		case 'properties_other_info':
			...
			_.forEach(this.data, function (item) {
				...
				this.text_x = Math.max(this.text_x, item.name.calc_width(panel.fonts.normal) + 20);
			}, this);
			_dispose(fileinfo);
			break;
		case 'queue_viewer':
	...
	this.init = function () {
		...
		case 'lastfm_info':
			this.get = function () {
				...
				var task_id = utils.GetRequestAsync(window.ID, url, lastfm.ua);
		...
		case 'musicbrainz':
			this.get = function () {
				...
				var task_id = utils.GetRequestAsync(window.ID, url, 'foo_jscript_panel_musicbrainz');
				this.filenames[task_id] = this.filename;
			}
